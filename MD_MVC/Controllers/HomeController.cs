...
namespace MD_MVC.Controllers
{
    public class HomeController : Controller
    {
        ...
        {
            contexto = new Modelos.MD_MVCEntities();
            using (RepositorioGenerico.Repositorio<Modelos.Condicione> p = new RepositorioGenerico.Repositorio<Modelos.Condicione>(contexto))
            {
                p.Excepcion += P_Excepcion;
                var datos = p.All().OrderBy(x => x.Descripcion);
                List<Select2> list = new List<Select2>();
        ...
        public ActionResult GuardarCondicion(string descripcion)
        {
            contexto = new Modelos.MD_MVCEntities();
            bool resultado = false;
            using (RepositorioGenerico.Repositorio<Modelos.Condicione> p = 
                new RepositorioGenerico.Repositorio<Modelos.Condicione>(contexto))
            {
                p.Excepcion += P_Excepcion;
                p.Create(new Modelos.Condicione { Descripcion = descripcion });
                resultado = true;
            }
            return Json(new { resultado });
        }
        ...
        public ActionResult ActualizarCondiciones(int id, string PropertyName, string value)
        {
            ...
            contexto = new Modelos.MD_MVCEntities();
            using (RepositorioGenerico.Repositorio<Modelos.Condicione> obj = 
                new RepositorioGenerico.Repositorio<Modelos.Condicione>(contexto))
            {
                obj.Excepcion += P_Excepcion;
                obj.Update(x => x.Id == id, PropertyName, value);
        ...
        public ActionResult ActualizarOrden(int id, string PropertyName, string value)
        {
            ...
            switch (PropertyName)
            {
                case "Id_Tipo":
                    ...
                    break;
                case "Id_Condiciones":
                    using (RepositorioGenerico.Repositorio<Modelos.Condicione> tipo = new RepositorioGenerico.Repositorio<Modelos.Condicione>(contexto))
                    {
                        int idValor = int.Parse(value);
                        value = tipo.Retrieve(x => x.Id == idValor).Descripcion;
                    }
                    break;
                case "Id_Estatus":
                    using (RepositorioGenerico.Repositorio<Modelos.Estatu> tipo = new RepositorioGenerico.Repositorio<Modelos.Estatu>(contexto))
                    {
                        int idValor = int.Parse(value);
                        value = tipo.Retrieve(x => x.Id == idValor).Descripcion;
                    }
        ...
        {
            contexto = new Modelos.MD_MVCEntities();
            using (RepositorioGenerico.Repositorio<Modelos.Estatu> p = new RepositorioGenerico.Repositorio<Modelos.Estatu>(contexto))
            {
                p.Excepcion += P_Excepcion;
                var datos = p.All().OrderBy(x => x.Descripcion);
                List<Select2> list = new List<Select2>();
        ...
        public string EstatusJSON()
        {
            StringBuilder sb = new StringBuilder();
            contexto = new Modelos.MD_MVCEntities();
            using (RepositorioGenerico.Repositorio<Modelos.Estatu> obj = new RepositorioGenerico.Repositorio<Modelos.Estatu>(contexto))
            {
                obj.Excepcion += P_Excepcion;
                var datos = obj.Filter(x => true);
                foreach (var item in datos)
